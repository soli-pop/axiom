...
/* ─── CONFIRMED STUDENTS (for admin pickers) ─────────────────────────────── */
// Returns all students who have confirmed their SN and have a linked profile.
// Used by admin enrollment and grade-encoding student selectors.
export const getConfirmedStudents = async () => {
  const { data, error } = await supabase
    .from('students')
    .select('id, name, program, year_level, status, user_id')
    .order('name');
  if (error) throw error;
  return data.map(r => ({
    id:      r.id,
