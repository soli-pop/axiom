...
import {
  ...
  Megaphone, Award, BookCheck, ChevronLeft
} from "lucide-react";
...
[data-theme="light"]{
  --base:#EDF0F5;--surf:#F5F7FB;--surf2:#E5E9F2;--surf3:#DAE0EC;
  --text:#1E2329;--muted:#687080;--dim:#98A0B0;
  --neo:-3px -3px 8px rgba(255,255,255,0.9),3px 3px 8px rgba(0,0,0,0.14);
  --neo-sm:-2px -2px 5px rgba(255,255,255,0.85),2px 2px 5px rgba(0,0,0,0.11);
  --neo-in:inset -2px -2px 5px rgba(255,255,255,0.75),inset 2px 2px 5px rgba(0,0,0,0.09)
}
...
[data-theme="light"] .neo-input{border-color:rgba(42,91,82,0.15)}
[data-theme="light"] .neo-input:focus{border-color:rgba(42,91,82,0.4)}
  ...
  <div className="modal-wrap" onClick={onCancel}>
    <div className="modal-card neo" style={{width:"100%",maxWidth:420,padding:28}} onClick={e=>e.stopPropagation()}>
      <div style={{display:"flex",alignItems:"flex-start",gap:14,marginBottom:20}}>
        ...
        <div>
          <div style={{fontFamily:"var(--ff-brand)",fontSize:18,fontWeight:600,marginBottom:5}}>{title}</div>
...
const SignupModal = ({onClose,onSuccess,onCreated}) => {
  ...
  const submit=async()=>{
    ...
    const existing = Object.values(getAllAccounts());
    if(existing.some(a => a.username?.toLowerCase() === username.toLowerCase())){setErr("Username already exists.");return;}
    if(existing.some(a => a.email?.toLowerCase() === email)){setErr("Email already exists.");return;}
    setLoading(true);
    ...
    const created = {
      ...
      dept: form.dept,
      pass: form.pass,
      avatar: (form.name.trim().split(/\s+/).map(p => p[0]).slice(0,2).join("") || username.slice(0,2)).toUpperCase(),
    };
    saveCustomAccount(created);
    ...
    <div className="modal-wrap" onClick={onClose}>
      <div className="modal-card neo" style={{width:"100%",maxWidth:430,padding:28,maxHeight:"90dvh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:22}}>
          <div>
            <div style={{fontFamily:"var(--ff-brand)",fontSize:20,fontWeight:700}}>Create Account</div>
        ...
        {done?(
          <div style={{textAlign:"center",padding:"24px 0"}}>
            ...
            <div style={{fontFamily:"var(--ff-brand)",fontSize:17,fontWeight:600,color:"#27AE60",marginBottom:6}}>Account Created!</div>
          ...
          {[{label:"Full Name",k:"name",type:"text",ph:"e.g. Maria Santos"},{label:"Username",k:"username",type:"text",ph:"e.g. maria123"},{label:"Email Address",k:"email",type:"email",ph:"e.g. familyuse2241@gmail.com"},{label:"ID Number",k:"idno",type:"text",ph:"e.g. SN-2025-001"}].map(({label,k,type,ph})=>(
...
const Login = ({onLogin,onBack}) => {
  ...
  const submit=async()=>{
    setErr("");
    const cred=findAccount(uname, role);
    if(!cred||cred.pass!==pass){setErr("Invalid username or password.");return;}
    if(cred.role!==role){setErr(`This account is not registered as a ${role}.`);return;}
    setLoading(true);
    try{
      const token=await apiSendOtp(cred.email);
      onLogin({...cred,username:cred.username || uname.trim().toLowerCase()},token);
    }catch(e){setErr(e.message||"Could not send verification code.");setLoading(false);}
  };
    ...
    <div style={{minHeight:"100dvh",display:"flex",alignItems:"center",justifyContent:"center",padding:20,background:"var(--base)",position:"relative"}}>
      ...
      <div style={{width:"100%",maxWidth:400,position:"relative",zIndex:1}}>
        <div className="fu" style={{textAlign:"center",marginBottom:32}}>
          ...
          <div style={{fontFamily:"var(--ff-brand)",fontSize:22,fontWeight:700}}>Secure Sign-In</div>
    ...
    <div style={{minHeight:"100dvh",display:"flex",alignItems:"center",justifyContent:"center",padding:20,background:"var(--base)",position:"relative"}}>
      ...
      <div style={{width:"100%",maxWidth:380,textAlign:"center",position:"relative",zIndex:1}}>
        <div className="fu" style={{marginBottom:26}}>
          ...
          <div style={{fontFamily:"var(--ff-brand)",fontSize:22,fontWeight:700}}>Check Your Email</div>
...
/* ────────────────── ADMIN OVERVIEW ────────────────── */
const AdminOverview = ({loading}) => {
  const tiles=[
    {label:"Total Students",  value:"1,240",sub:"+24 this sem",  icon:<Users size={20}/>,       c:"var(--mint)"},
    ...
    {label:"School GPA Avg",  value:"1.82", sub:"Target ≤ 1.80", icon:<BarChart3 size={20}/>,    c:"#27AE60"},
    {label:"Enrolled This Sem",value:"1,192",sub:"95.8% rate",   icon:<BookCheck size={20}/>,    c:"#9B59B6"},
  ...
  return (
    <div>
      <div className="neo fu" style={{padding:"16px 20px",marginBottom:16,borderLeft:"3px solid var(--acc)",display:"flex",alignItems:"center",gap:12}}>
        <div style={{width:38,height:38,background:"var(--acc)",borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"var(--ff-brand)",fontWeight:700,fontSize:15,color:"var(--mint)",flexShrink:0}}>AR</div>
        <div>
          <div style={{fontFamily:"var(--ff-brand)",fontSize:17,fontWeight:600}}>Good day, Rector Reyes.</div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:12}}>
        ...
        <div className="neo fu d4" style={{padding:"18px 20px"}}>
          ...
          {loading?<><Skel h={12} mb={8}/><Skel h={12} mb={8}/><Skel h={12}/></>:
            ANN.filter(a=>a.tag==="URGENT"||a.tag==="DEADLINE").map(a=>(
...
/* ────────────────── TEACHER OVERVIEW ────────────────── */
const TeacherOverview = ({loading}) => {
  const mySubjects=SUBJECTS_FULL.filter(s=>s.instructor==="Prof. M. Luna");
  return (
    <div>
      <div className="neo fu" style={{padding:"16px 20px",marginBottom:16,borderLeft:"3px solid var(--acc)"}}>
        <div style={{fontFamily:"var(--ff-brand)",fontSize:17,fontWeight:600}}>Good day, Prof. Luna.</div>
          ...
          mySubjects.map((s,i)=>(
            <div key={s.code} className={`neo fu d${i+1}`} style={{padding:"18px 20px"}}>
              ...
              <div style={{display:"flex",gap:10,marginTop:10}}>
                <div style={{fontSize:12,color:"var(--muted)"}}>Students: <span style={{color:"var(--mint)",fontWeight:500}}>4</span></div>
                <div style={{fontSize:12,color:"var(--muted)"}}>Avg Grade: <span style={{color:"#27AE60",fontWeight:500}}>86.5</span></div>
              </div>
            </div>
          ))
        }
      ...
      <div className="neo fu d4" style={{padding:"18px 20px"}}>
        <div style={{fontSize:11.5,color:"var(--muted)",marginBottom:14,fontWeight:500,letterSpacing:"0.06em",textTransform:"uppercase",display:"flex",alignItems:"center",gap:8}}><Users size={14}/>Student Roster — CS301</div>
        {loading?<><Skel h={12} mb={8}/><Skel h={12} mb={8}/><Skel h={12}/></>:
          <table className="dt">
            <thead><tr><th>ID</th><th>Student</th><th>Quiz</th><th>Exam</th><th>Grade</th><th>Status</th></tr></thead>
            <tbody>
              {GRADE_DATA.Prelim.map(r=>{
...
/* ────────────────── STUDENT OVERVIEW ────────────────── */
const StudentOverview = ({loading}) => {
  const enrolled=SUBJECTS_FULL.filter(s=>ENROLLED_DEFAULT.includes(s.code));
  const totalUnits=enrolled.reduce((a,s)=>a+s.units,0);
  return (
    <div>
      <div className="neo fu" style={{padding:"16px 20px",marginBottom:16,borderLeft:"3px solid var(--acc)"}}>
        <div style={{fontFamily:"var(--ff-brand)",fontSize:17,fontWeight:600}}>Good day, Alexa.</div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(155px,1fr))",gap:12,marginBottom:16}}>
        {loading?[0,1,2,3].map(i=><div key={i} className="neo" style={{padding:18}}><Skel h={13} mb={8}/><Skel w="60%" h={22} mb={6}/></div>):
          [
            ...
            {label:"Current GPA",   value:"1.65",     sub:"Prelim period",    c:"#27AE60",  icon:<TrendingUp size={18}/>},
            {label:"Account Balance",value:"₱8,450",  sub:"Due this month",  c:"#E74C3C",  icon:<Receipt size={18}/>},
    ...
    <div>
      ...
      <div className="neo" style={{overflow:"auto"}}>
        <table className="dt">
          ...
          <tbody>
            {filtered.length===0&&<tr><td colSpan={readOnly?6:7} style={{textAlign:"center",color:"var(--muted)",padding:24,fontSize:13}}>No students found.</td></tr>}
            {filtered.map(s=>(
              <tr key={s.id}>
                ...
                <td><span style={{fontFamily:"var(--ff-mono)",fontSize:12.5,color:s.gpa<1.75?"#27AE60":s.gpa<2.5?"var(--mint)":"#D4A017"}}>{s.gpa.toFixed(2)}</span></td>
                ...
                {!readOnly&&<td>
                  <div style={{display:"flex",gap:6,justifyContent:"flex-end"}}>
                    ...
                    <button className="btn btn-icon btn-sm" style={{background:"rgba(192,57,43,0.12)",color:"#E74C3C"}} onClick={()=>showModal({title:"Delete Record",msg:`Remove ${s.name}?`,danger:true,confirmLabel:"Delete",onConfirm:()=>{setStudents(p=>p.filter(x=>x.id!==s.id));toast(`${s.name} removed.`,"error");}})}><Trash2 size={12}/></button>
                  </div>
                </td>}
              </tr>
            ))}
      ...
      {form&&<div className="modal-wrap" onClick={()=>setForm(null)}><div className="modal-card neo" style={{width:"100%",maxWidth:420,padding:26}} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}><div style={{fontFamily:"var(--ff-brand)",fontSize:18,fontWeight:600}}>{form.mode==="add"?"New Student":"Edit Record"}</div><button className="btn btn-ghost btn-icon" onClick={()=>setForm(null)}><X size={15}/></button></div>
        ...
        <div style={{display:"flex",gap:10,justifyContent:"flex-end",marginTop:8}}>
          ...
          <button className="btn btn-pri btn-sm" onClick={()=>{
            ...
            if(form.mode==="add")setStudents(p=>[...p,{...draft,yr:Number(draft.yr),gpa:Number(draft.gpa)}]);
            else setStudents(p=>p.map(s=>s.id===form.orig.id?{...draft,yr:Number(draft.yr),gpa:Number(draft.gpa)}:s));
            toast(form.mode==="add"?"Student record added.":"Record updated.");setForm(null);
...
const GradesView = ({showModal,toast}) => {
  ...
  return (
    <div>
      ...
      <div className="neo" style={{overflow:"auto",marginBottom:14}}>
        <table className="dt">
              ...
              return (
                <tr key={r.id}>
                  ...
                  {["quiz","exam","proj"].map(f=>(
                    <td key={f}><input style={{width:56,background:"var(--base)",boxShadow:"var(--neo-in)",border:"1px solid rgba(163,209,198,0.08)",borderRadius:6,color:"var(--text)",fontFamily:"var(--ff-mono)",fontSize:12,padding:"4px 7px",outline:"none",textAlign:"center",transition:"border-color .15s"}} type="number" min={0} max={100} value={r[f]} onChange={e=>setGrades(g=>({...g,[period]:g[period].map(x=>x.id===r.id?{...x,[f]:Number(e.target.value)}:x)}))} onFocus={e=>e.target.style.borderColor="rgba(163,209,198,0.35)"} onBlur={e=>e.target.style.borderColor="rgba(163,209,198,0.08)"}/></td>
                  ))}
      ...
      <div style={{display:"flex",justifyContent:"flex-end",gap:10}}>
        ...
        <button className="btn btn-pri btn-sm" onClick={()=>showModal({title:"Finalize Grades",msg:`Submit ${period} grades for ${subject}? This cannot be modified after submission.`,confirmLabel:"Finalize",onConfirm:()=>toast(`${period} grades for ${subject} submitted.`)})}><Check size={12}/>Finalize {period}</button>
      </div>
    </div>
  );
};
...
/* ────────────────── ENROLLMENT VIEW ────────────────── */
const EnrollmentView = ({toast, role}) => {
  const sorted=[...SUBJECTS_FULL].sort((a,b)=>a.yr-b.yr||a.code.localeCompare(b.code));
  ...
  const enrolledSubjects=SUBJECTS_FULL.filter(s=>enrolled.includes(s.code)).sort((a,b)=>a.yr-b.yr||a.code.localeCompare(b.code));
  const totalUnits=enrolledSubjects.reduce((a,s)=>a+s.units,0);

  const available=sorted.filter(s=>
    !enrolled.includes(s.code)&&
    (dept===""||s.dept===dept)&&
    (filterYr===0||s.yr===filterYr)
  );

  const add=code=>{
    if(enrolled.includes(code)){toast(`${code} already enrolled.`,"warn");return;}
    setEnrolled(p=>[...p,code]);toast(`${code} added to enrollment.`);
  };
  const drop=code=>{setEnrolled(p=>p.filter(x=>x!==code));toast(`${code} dropped.`,"warn");};
              ...
              <tbody>
                {enrolledSubjects.map(s=>(
                  <tr key={s.code}>
                    ...
                    {role!=="admin"&&<td><button className="btn btn-icon btn-sm" style={{background:"rgba(192,57,43,0.1)",color:"#E74C3C"}} onClick={()=>drop(s.code)} title="Drop"><X size={11}/></button></td>}
                  </tr>
                ))}
              </tbody>
            </table>
  ...
  return (
    <div>
      ...
      <div className="neo fu" style={{padding:"20px 24px",marginBottom:16,display:"flex",alignItems:"center",gap:16,flexWrap:"wrap"}}>
        <div style={{flex:1}}>
          <div style={{fontFamily:"var(--ff-brand)",fontSize:20,fontWeight:700}}>Statement of Account</div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:14}}>
        ...
        <div className="neo fu d3" style={{padding:"18px 20px"}}>
          ...
          <button className="btn btn-pri btn-sm" style={{width:"100%",marginTop:14}}><Receipt size={12}/>Print SOA</button>
        </div>
      </div>
    </div>
  );
      ...
      {(cat==="all"||cat==="board")&&(
        <div className="neo fu" style={{padding:"18px 20px",marginBottom:14,borderLeft:"3px solid #F39C12",background:"rgba(243,156,18,0.05)"}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
            ...
            <span style={{fontFamily:"var(--ff-brand)",fontSize:16,fontWeight:700,color:"#F39C12"}}>Board Passers — AY 2025–2026</span>
          </div>
        ...
        {filtered.map((a,i)=>(
          <div key={a.id} className={`neo-sm fu d${(i%4)+1}`} style={{padding:"16px 18px",borderLeft:`3px solid ${tagC[a.tag]||"var(--mint)"}`}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:7}}>
              <Badge label={a.tag}/>
              <span style={{fontFamily:"var(--ff-brand)",fontSize:15,fontWeight:600}}>{a.title}</span>
            </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:14}}>
        {filtered.map((ev,i)=>(
          <div key={ev.id} className={`neo fu d${(i%4)+1}`} style={{padding:"20px 20px",borderTop:`3px solid ${typeColor[ev.type]||"var(--mint)"}`}}>
            ...
            <div style={{fontFamily:"var(--ff-brand)",fontSize:16,fontWeight:700,marginBottom:6,lineHeight:1.3}}>{ev.title}</div>
    ...
    <div style={{maxWidth:620}}>
      ...
      <div className="neo fu" style={{padding:"22px 24px",marginBottom:16}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:20,marginBottom:22}}>
          ...
          <div>
            <div style={{fontFamily:"var(--ff-brand)",fontSize:17,fontWeight:600}}>{user.name}</div>
...
const Dashboard = ({user, setUser, theme, setTheme, onLogout, showModal, toast}) => {
  ...
  useEffect(()=>{setLoading(true);const t=setTimeout(()=>setLoading(false),1300);return()=>clearTimeout(t);},[active]);
  ...
  const Content=()=>{
    if(loading) return (
      <div style={{padding:"20px 0"}}>
        <Skel h={18} mb={10}/><Skel w="80%" h={14} mb={20}/>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:12,marginBottom:16}}>
          {[0,1,2,3].map(i=><div key={i} className="neo" style={{padding:18}}><Skel h={13} mb={8}/><Skel w="60%" h={24} mb={6}/><Skel w="80%" h={11}/></div>)}
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:12}}>
          {[0,1].map(i=><div key={i} className="neo" style={{padding:18,minHeight:130}}><Skel h={12} mb={10}/><Skel h={12} mb={8}/><Skel h={12} mb={8}/><Skel w="70%" h={12}/></div>)}
        </div>
      </div>
    );
    const p={showModal,toast};
    if(active==="overview"){
      if(user.role==="admin")   return <AdminOverview loading={false}/>;
      if(user.role==="teacher") return <TeacherOverview loading={false}/>;
      return <StudentOverview loading={false}/>;
    }
    if(active==="students") return <StudentsView {...p} readOnly={user.role==="teacher"}/>;
    if(active==="grades")   return <GradesView {...p}/>;
    if(active==="enroll")   return <EnrollmentView toast={toast} role={user.role}/>;
    if(active==="soa")      return <SOAView role={user.role}/>;
...
export default function App() {
  ...
  const handleLogout = () => showModal({
    ...
    onConfirm:()=>{ closeModal(); setUser(null); setOtpToken(""); setScreen("hero"); }
  });
