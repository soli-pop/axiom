...
const SignupModal = ({onClose,onSuccess,onCreated}) => {
  ...
  // Reset form when role switches
  const switchRole=r=>{setRole(r);setForm({name:"",username:"",email:"",dept:"Computer Science",pass:"",confirm:""});setErr("");};
  ...
  const Field=({label,k,type="text",ph,hint})=>(
    ...
    <div className="modal-wrap" onClick={onClose}>
      <div className="modal-card neo" style={{width:"100%",maxWidth:430,padding:28,maxHeight:"90dvh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
        ...
        {done?(
          ...
          {/* Common fields */}
          <Field label="Full Name *"      k="name"     ph={role==="teacher"?"e.g. Prof. Juan Dela Cruz":"e.g. Maria Santos"}/>
          <Field label="Username *"       k="username" ph="e.g. maria123" hint="Used to sign in. Lowercase, no spaces."/>
          <Field label="Email Address *"  k="email"    type="email" ph="example@email.com" hint="OTP verification will be sent here."/>

          {/* Teacher-only fields */}
          {role==="teacher"&&<>
            <Field label="Employee ID"    k="empId"    ph="e.g. EMP-2025-001"/>
            <div style={{marginBottom:14}}>
...
const Login = ({onLogin,onBack}) => {
  ...
  const submit=async()=>{
    ...
    try{
      ...
      // Admin accounts bypass the role selector — they go straight through
