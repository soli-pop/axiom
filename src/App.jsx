...
/* ────────────────── STYLES ────────────────── */
// Defined as a constant outside any component — never re-created, injected once via useEffect in App.
const STYLES = `
...
/* ────────────────── SIGNUP FIELD ────────────────── */
// FIX: Moved outside SignupModal. When defined inside a component, React treats it as a
// brand-new component type on every render, unmounting+remounting the input and causing
// the "one-letter delay / loss of focus" bug.
const SignupField = ({ label, k, type="text", ph, hint, value, onChange, status }) => (
  <div style={{marginBottom:14}}>
    <div style={{fontSize:11.5,color:"var(--muted)",marginBottom:7,fontWeight:500,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      <span>{label}</span>
      {status==="checking" && <span style={{fontSize:10,color:"var(--muted)",display:"flex",alignItems:"center",gap:4}}><Spin size={10}/>checking…</span>}
      {status==="taken"    && <span style={{fontSize:10,color:"var(--danger)",fontWeight:600}}>already taken ✗</span>}
      {status==="ok"       && <span style={{fontSize:10,color:"var(--success)",fontWeight:600}}>available ✓</span>}
    </div>
    <input
      className="neo-input"
      type={type}
      ...
      onChange={onChange}
      style={
        status==="taken" ? {borderColor:"rgba(192,57,43,0.5)"} :
        status==="ok"    ? {borderColor:"rgba(39,174,96,0.4)"} :
        {}
      }
    />
...
const SignupModal = ({onClose, onSuccess, onCreated}) => {
  ...
  const [form,    setForm]    = useState({name:"",username:"",email:"",dept:"Computer Science",pass:"",confirm:""});
  ...
  // "" = idle, "checking" = debounce fired, "ok" = available, "taken" = conflict
  const [unameStatus, setUnameStatus] = useState("");
  const [emailStatus, setEmailStatus] = useState("");

  // FIX: useCallback so `set` is a stable reference across renders,
  // preventing child re-render churn from prop reference changes.
  const set = useCallback(k => e => setForm(p => ({...p, [k]: e.target.value})), []);

  const switchRole = r => {
    setRole(r);
    setForm({name:"",username:"",email:"",dept:"Computer Science",pass:"",confirm:""});
    setErr("");
    setUnameStatus("");
    setEmailStatus("");
  };

  // ── Debounced username availability check (600ms) ────────────────
  useEffect(() => {
    if (!form.username || form.username.trim().length < 3) { setUnameStatus(""); return; }
    ...
    const t = setTimeout(async () => {
      try {
        const { data } = await supabase
          ...
          .maybeSingle();
        setUnameStatus(data ? "taken" : "ok");
      } catch {
        setUnameStatus(""); // fail silently — submit will catch it
      }
    }, 600);
  ...
  useEffect(() => {
    ...
    if (!form.email || !emailRegex.test(form.email.trim())) { setEmailStatus(""); return; }
  ...
  const submit = async () => {
    setErr("");
    if (!form.name||!form.username||!form.email||!form.pass) { setErr("Please fill in all required fields."); return; }
    ...
    if (emailStatus === "taken")                              { setErr("That email is already registered."); return; }
    setLoading(true);
    try {
        ...
        {done ? (
          <div style={{textAlign:"center",padding:"24px 0"}}>
            ...
            <div style={{fontSize:12.5,color:"var(--muted)",lineHeight:1.6}}>
              {role==="student"
                ? "Your account is pending. An admin will assign your Student Number before you can access the portal."
                : "Your account is ready. You may now sign in."}
            </div>
          </div>
        ) : <>
          {/* Role selector */}
          <div style={{marginBottom:20}}>
            <div style={{fontSize:11,color:"var(--muted)",marginBottom:8,letterSpacing:"0.08em",textTransform:"uppercase",fontWeight:500}}>I am a…</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              {[
                {r:"student", label:"Student",  desc:"Enrollee of the institution"},
                {r:"teacher", label:"Faculty",   desc:"Instructor / staff member"},
              ].map(({r,label,desc})=>(
                <div key={r} onClick={()=>switchRole(r)} style={{padding:"12px 14px",borderRadius:10,border:`1.5px solid ${role===r?"rgba(163,209,198,0.35)":"rgba(163,209,198,0.08)"}`,background:role===r?"rgba(42,91,82,0.14)":"transparent",cursor:"pointer",transition:"all .18s"}}>
                  <div style={{fontSize:13,fontWeight:600,color:role===r?"var(--mint)":"var(--text)",marginBottom:3}}>{label}</div>
          ...
          {/* Common fields — using SignupField (defined outside, stable reference) */}
          <SignupField label="Full Name *"     k="name"     ph={role==="teacher"?"e.g. Prof. Juan Dela Cruz":"e.g. Maria Santos"} value={form.name}     onChange={set("name")}/>
          <SignupField label="Username *"      k="username" ph="e.g. maria123" hint="Used to sign in. Lowercase, no spaces." value={form.username} onChange={set("username")} status={unameStatus}/>
          <SignupField label="Email Address *" k="email"    type="email" ph="example@email.com" hint="OTP verification will be sent here." value={form.email} onChange={set("email")} status={emailStatus}/>

          {/* Teacher-only fields */}
          {role==="teacher" && <>
            <SignupField label="Employee ID" k="empId" ph="e.g. EMP-2025-001" value={form.empId||""} onChange={set("empId")}/>
            <div style={{marginBottom:14}}>
              <div style={{fontSize:11.5,color:"var(--muted)",marginBottom:7,fontWeight:500}}>Department / College</div>
          ...
          {/* Student notice */}
          {role==="student" &&
            <div style={{marginBottom:14,padding:"10px 13px",background:"rgba(212,160,23,0.08)",border:"1px solid rgba(212,160,23,0.2)",borderRadius:8,display:"flex",gap:9,alignItems:"flex-start"}}>
              <AlertTriangle size={14} color="#D4A017" style={{flexShrink:0,marginTop:1}}/>
              <div style={{fontSize:12,color:"var(--muted)",lineHeight:1.5}}>
                <strong style={{color:"#D4A017"}}>Note:</strong> After signing up, an administrator must assign your Student Number before you can access the portal. You'll be prompted to enter it upon your first login.
              </div>
            </div>
          }
          ...
          <button className="btn btn-pri" style={{width:"100%",padding:"12px",fontSize:13.5,borderRadius:10}} onClick={submit} disabled={loading||unameStatus==="checking"||emailStatus==="checking"}>
  ...
  const submit=async()=>{
    ...
    try{
      ...
      if(user.role!=="admin" && user.role!==role){
        setErr(`This account is not registered as ${role==="teacher"?"faculty":"a student"}.`);
      ...
      <div style={{width:"100%",maxWidth:400,position:"relative",zIndex:1}}>
        ...
        <div className="neo fu d2" style={{padding:28}}>
          <div style={{marginBottom:22}}>
            <div style={{fontSize:11,color:"var(--muted)",marginBottom:8,letterSpacing:"0.08em",textTransform:"uppercase",fontWeight:500}}>Sign in as</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              {[
                {r:"student", label:"Student"},
                {r:"teacher", label:"Faculty"},
              ].map(({r,label})=>(
                <div key={r}><input id={`r-${r}`} className="ri" type="radio" name="role" value={r} checked={role===r} onChange={()=>setRole(r)}/>
                <label htmlFor={`r-${r}`} style={{justifyContent:"center",padding:"9px 6px",fontSize:13}}><span className="rdot"/>{label}</label></div>
              ))}
            </div>
          </div>
          <div style={{marginBottom:14}}>
...
const StudentsView = ({showModal,toast,readOnly=false}) => {
  ...
  // FIX: F was previously defined inside this component, causing remount on every
  // keystroke in the draft fields. Now it's an inline render function (not a component).
  const renderField = (label, k, type="text", opts) => (
    <div key={k} style={{marginBottom:12}}>
      ...
      {opts
        ? <select className="neo-input" value={draft[k]||""} onChange={e=>setDraft(p=>({...p,[k]:e.target.value}))} style={{appearance:"none"}}>{opts.map(o=><option key={o} value={o}>{o}</option>)}</select>
        : <input className="neo-input" type={type} value={draft[k]||""} onChange={e=>setDraft(p=>({...p,[k]:e.target.value}))}/>
      }
    </div>
  );
  ...
  return (
    <div>
      ...
      {form&&<div className="modal-wrap" onClick={()=>setForm(null)}><div className="modal-card neo" style={{width:"100%",maxWidth:420,padding:26}} onClick={e=>e.stopPropagation()}>
        ...
        {renderField("Program *","program",undefined,["BS Computer Science","BS Information Technology","BS Mathematics","BS Physics","BS Biology"])}
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          {renderField("Year Level","yr","number")}
          {renderField("GPA","gpa","number")}
        </div>
        {renderField("Status","status",undefined,["Enrolled","Probation","LOA","Graduated","Dropped"])}
...
const GradesView = ({showModal,toast,user}) => {
  ...
  // Admin-only: student picker for adding a grade row
  ...
  useEffect(()=>{
    getSubjects().then(all=>{
      let filtered = isAdmin ? all : all.filter(s=>s.instructor===user?.name||s.instructor==="Prof. M. Luna");
      setSubjects(filtered);
      if(filtered.length && !subject) setSubject(filtered[0].code);
    }).catch(()=>{
      const fb=SUBJECTS_FULL.filter(s=>isAdmin||s.instructor==="Prof. M. Luna");
      setSubjects(fb);
      if(fb.length && !subject) setSubject(fb[0].code);
    });
  },[]);
  ...
  const handleAddStudentRow=async()=>{
    if(!addStudentId){toast("Select a student first.","warn");return;}
    if(grades.find(r=>r.id===addStudentId)){toast("Student already has a grade entry for this subject/period.","warn");return;}
    setAddingRow(true);
    try{
      await saveGrade({sbId:null,studentId:addStudentId,subjectCode:subject,period,quiz:0,exam:0,proj:0});
      const stu=studentList.find(s=>s.id===addStudentId);
      setGrades(g=>[...g,{id:addStudentId,name:stu?.name||addStudentId,quiz:0,exam:0,proj:0,_sbId:null}]);
      setAddStudentId("");
      toast(`Grade row added for ${stu?.name||addStudentId}.`);
    }catch(e){toast("Failed to add row: "+e.message,"error");}
  ...
  const subjectList=subjects.length?subjects:(isAdmin?SUBJECTS_FULL:SUBJECTS_FULL.filter(s=>s.instructor==="Prof. M. Luna"));
  ...
  return (
    <div>
      ...
      {/* Admin: add a student grade row */}
      {isAdmin&&(
        <div className="neo-sm" style={{display:"flex",gap:10,alignItems:"center",padding:"10px 14px",marginBottom:14,flexWrap:"wrap"}}>
          <UserCheck size={14} color="var(--mint)"/>
          <span style={{fontSize:12.5,color:"var(--muted)",fontWeight:500}}>Add student to grade list:</span>
          <select
            className="neo-input"
            value={addStudentId}
            onChange={e=>setAddStudentId(e.target.value)}
            style={{appearance:"none",flex:1,minWidth:180,padding:"6px 11px",fontSize:12}}
          >
            <option value="">— Select student —</option>
            {studentList.filter(s=>!grades.find(g=>g.id===s.id)).map(s=>(
              <option key={s.id} value={s.id}>{s.name} ({s.id})</option>
            ))}
          </select>
          <button className="btn btn-pri btn-sm" onClick={handleAddStudentRow} disabled={addingRow||!addStudentId}>
            {addingRow?<Spin size={12}/>:<Plus size={12}/>}Add Row
          </button>
        </div>
      )}
      ...
      <div className="neo" style={{overflowX:"auto",marginBottom:14}}>
        <table className="dt" style={{minWidth:560}}>
          ...
          <tbody>
            ...
            {!loadingGrades&&grades.length===0&&<tr><td colSpan={6} style={{textAlign:"center",color:"var(--muted)",padding:20,fontSize:13}}>No grade records for this subject/period.{isAdmin&&" Use the selector above to add students."}</td></tr>}
...
const EnrollmentView = ({toast, role, user}) => {
  ...
  const effectiveStudentId = isAdmin ? selectedStudentId : (user?.idno||"");
  ...
  useEffect(()=>{
    ...
    getConfirmedStudents()
      .then(list=>{ setStudentList(list); setLoadingStudents(false); })
      .catch(()=>{ setStudentList([]); setLoadingStudents(false); });
  },[isAdmin]);

  useEffect(()=>{
    if(!effectiveStudentId){ setEnrolledItems([]); return; }
    setLoadingEnroll(true);
    getEnrollment(effectiveStudentId, semester)
      .then(setEnrolledItems)
      .catch(()=>setEnrolledItems([]))
      .finally(()=>setLoadingEnroll(false));
  },[effectiveStudentId,semester]);
  ...
  const add=async(code)=>{
    if(!effectiveStudentId){ toast("Select a student first.","warn"); return; }
    if(enrolledCodes.includes(code)){toast(`${code} already enrolled.`,"warn");return;}
    try{
      await enrollSubject(effectiveStudentId, code, semester);
  ...
  return (
    <div>
      {isAdmin&&(
        <div className="neo-sm" style={{padding:"14px 16px",marginBottom:20,display:"flex",flexWrap:"wrap",gap:12,alignItems:"center"}}>
          <UserCheck size={15} color="var(--mint)"/>
          <span style={{fontSize:13,fontWeight:600,color:"var(--text)"}}>Student Account</span>
          <select
            className="neo-input"
            value={selectedStudentId}
            onChange={e=>{setSelectedStudentId(e.target.value);setEnrolledItems([]);}}
            style={{appearance:"none",flex:1,minWidth:220,padding:"7px 12px",fontSize:12}}
            disabled={loadingStudents}
          >
            <option value="">{loadingStudents?"Loading students…":"— Select a registered student —"}</option>
            {studentList.map(s=>(
              <option key={s.id} value={s.id}>{s.name} · {s.id} · {s.program}</option>
            ))}
          </select>
          {selectedStudent&&(
            <span style={{fontSize:11.5,color:"var(--muted)",background:"var(--surf2)",borderRadius:6,padding:"4px 10px"}}>
              Yr {selectedStudent.yr} · <Badge label={selectedStudent.status}/>
            </span>
          )}
        </div>
      )}
      ...
      <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:18,flexWrap:"wrap"}}>
        <span style={{fontSize:12,color:"var(--muted)",fontWeight:500}}>Semester:</span>
      ...
      <div style={{marginBottom:20}}>
        ...
        {isAdmin&&!selectedStudentId?(
          <div className="neo" style={{padding:24,textAlign:"center",color:"var(--muted)",fontSize:13}}>Select a student above to view and manage their enrollment.</div>
        ):loadingEnroll?(
...
const Dashboard = ({user, setUser, theme, setTheme, onLogout, showModal, toast}) => {
  ...
  // FIX: Content, Avatar, SidebarContent were previously defined as components
  // (const X = () => ...) inside Dashboard. React treats each render's new function
  // reference as a different component type, causing full unmount+remount on every
  // navigation click. Changed to render functions (called as {renderX()}) so React
  // sees stable JSX without component identity churn.
  ...
  return (
    <div style={{display:"flex",minHeight:"100dvh"}}>
      ...
      <main style={{flex:1,display:"flex",flexDirection:"column",minWidth:0,paddingBottom:"var(--bnav)"}}>
        ...
        <div style={{flex:1,padding:18,maxWidth:1280}}>{renderContent()}</div>
      </main>
...
export default function App() {
  ...
  // FIX: Inject styles once into a real <style> tag in <head>, not into JSX.
  // When STYLES was rendered as <style>{STYLES}</style> inside the component tree,
  // React had to diff the entire multi-KB CSS string on every render. Now it runs
  // exactly once on mount and is never touched again.
  useEffect(() => {
    const el = document.createElement("style");
    el.setAttribute("data-axiom", "styles");
